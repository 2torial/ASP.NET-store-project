...
public class StorePriceCalculatorTest
{
    ...
    [Test]
    public void TestCalculate_BaseProfitLessThanOrEqual5_CleanReminders() {
        (decimal, decimal)[] testCases = [(1, 5), (2.5m, 2), (4, 0.25m), (3, 0.01m), (100, 0.04m), (100, 0.05m)];

        foreach (var (cost, multiplier) in testCases) Assert.That(
            StorePriceCalculator.Calculate(cost, multiplier),
            Is.EqualTo(cost + 5),
            $"cost:{cost}|multiplier:{multiplier}");
    }

    [Test]
    public void TestCalculate_BaseProfitLessThanThanOrEqual5_WithoutCleanReminders()
    {
    ...
    public void TestCalculate_BaseProfitHigherThan5() {
        ...
        foreach (var (cost, multiplier) in testCases)
        {
            foreach (var modifier in modifiers)
            {
                ...
                if (cleanReminders.Contains(reminder)) Assert.That(
                    StorePriceCalculator.Calculate(modifiedCost, multiplier), 
                    Is.EqualTo(result),
